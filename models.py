...
class Screen(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    # FIX: Added unique=True so you don't accidentally make two "Screen 1"s
...
class Seat(db.Model):
    ...
    # FIX: Prevents accidentally creating "Row A, Seat 1" twice in the same screen
    __table_args__ = (
        db.UniqueConstraint('screen_id', 'row', 'number', name='unique_seat_per_screen'),
    )
...
class BookingSeat(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('showtime_id', 'seat_id', name='unique_showtime_seat'),
    )